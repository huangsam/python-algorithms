    ...
    def match(pattern: str, text: str) -> bool:
        ...
        if len(pattern) >= 2 and pattern[1] in {"*", "+", "?"}:
            ...
            if quantifier == "*":
                ...
                return match(remaining_pattern, text) or (first_match and match(pattern, text[1:]))
            elif quantifier == "+":
                # Must have at least one match, then acts like *
                return first_match and match(pattern[0] + "*" + remaining_pattern, text[1:])
            elif quantifier == "?":
                # 0 matches (skip) OR 1 match (consume and move on)
                return match(remaining_pattern, text) or (first_match and match(remaining_pattern, text[1:]))
